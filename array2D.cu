...
__device__ double * numjac(double *resp, double bc0, double bc1, double bc2, double bc3, double yb0, double yb1, double yb2, double yb3, int Nt0, int pi, int pf, int pij, double *J) {
	...
	i = 0;
	bcaux = bc0 - dy;
	resp = rk4(bcaux, bc1, bc2, bc3, resp, pi, pf);
	...
	i = 0;
	bcaux = bc1 - dy;
	resp = rk4(bc0, bcaux, bc2, bc3, resp, pi, pf);
	v1 = resp[num + i - 7 + pi] - yb0;
	...
	i = 0;
	bcaux = bc2 - dy;
	resp = rk4(bc0, bc1, bcaux, bc3, resp, pi, pf);
	v1 = resp[num + i - 7 + pi] - yb0;
	...
	i = 0;
	bcaux = bc3 - dy;
	resp = rk4(bc0, bc1, bc2, bcaux, resp, pi, pf);
	v1 = resp[num + i - 7 + pi] - yb0;
...
__device__ double bvpsol(double *resp, double *J, double yb0, double yb1, double yb2, double yb3, int Nt0, int pi, int pf, int pij){
	...
	J[0+pij] = 0.0; J[1+pij] = 0.0; J[2+pij] = 0.0; J[3+pij] = 0.0;
	J[4+pij] = 0.0; J[5+pij] = 0.0; J[6+pij] = 0.0; J[7+pij] = 0.0;
	J[8+pij] = 0.0; J[9+pij] = 0.0; J[10+pij] = 0.0;J[11+pij] = 0.0;
	J[12+pij] = 0.0;J[13+pij] = 0.0;J[14+pij] = 0.0;J[15+pij] = 0.0;

	runs = 1;
	for (int i = 0;i < Nmax;i++) {

		J = numjac(resp, bc0, bc1, bc2, bc3, yb0, yb1, yb2, yb3, Nt0, pi, pf, pij, J);

		J11 = J[0+pij];  J12 = J[1+pij];  J13 = J[2+pij]; J14 = J[3+pij];
		J21 = J[4+pij];  J22 = J[5+pij];  J23 = J[6+pij]; J24 = J[7+pij];
		J31 = J[8+pij];  J32 = J[9+pij];  J33 = J[10+pij];J34 = J[11+pij];
		J41 = J[12+pij]; J42 = J[13+pij]; J43 = J[14+pij];J44 = J[15+pij];
		...
		resp = rk4(bc0, bc1, bc2, bc3, resp, pi, pf);
...
//Kernel
__global__ void shoot(double *c, double *X1, double *X2, double *X3, double *X4, double *resp, double *J){
	...
	pi = index * Nt0; pf = (index + 1)*Nt0 - 1;
        pij = index * NJ; pfj = (index + 1)*NJ - 1;
	c[index] = bvpsol(resp, J, x1, x2, x3, x4, Nt0, pi, pf, pij);
}
...
int main(void){
    ...
    int size_J = 16*N * sizeof(double);
    int size_resp = N*Nt * sizeof(double);
    int size_int = sizeof(int);

    double *c, *X1, *X2, *X3, *X4, *resp, *J;
    double *d_c, *d_X1, *d_X2, *d_X3, *d_X4, *d_resp, *d_J;

    FILE * fp;
    fp = fopen ("GPU2DOF.txt", "w");
    ...
    cudaMalloc((void **)&d_J, size_J);
    cudaMalloc((void **)&d_c, size_N);
    cudaMalloc((void **)&d_X1, size_N);
    cudaMalloc((void **)&d_X2, size_N);
    cudaMalloc((void **)&d_X3, size_N);
    cudaMalloc((void **)&d_X4, size_N);
    cudaMalloc((void **)&d_resp, size_resp);
    ...
    cudaMemcpy(d_X1, X1, size_N, cudaMemcpyHostToDevice);
    cudaMemcpy(d_X2, X2, size_N, cudaMemcpyHostToDevice);
    cudaMemcpy(d_X3, X3, size_N, cudaMemcpyHostToDevice);
    cudaMemcpy(d_X4, X4, size_N, cudaMemcpyHostToDevice);
    cudaMemcpy(d_resp, resp, size_resp, cudaMemcpyHostToDevice);
    cudaMemcpy(d_J, J, size_J, cudaMemcpyHostToDevice);

    // Launch kernel on GPU
    cudaEventRecord(start);
    shoot<<<256,256>>>(d_c, d_X1, d_X2, d_X3, d_X4, d_resp, d_J);  
    cudaEventRecord(stop);
    
    // Copy result back to host
    cudaMemcpy(c, d_c, size_N, cudaMemcpyDeviceToHost);
    ...
    cudaFree(d_c); cudaFree(d_X1); cudaFree(d_X2); cudaFree(d_X3); cudaFree(d_X4); cudaFree(d_resp); cudaFree(d_J);
