...
__device__ double * OdeFun(double t0, double y0, double y1, double y2, double y3){
	...
	double exp1;
        ...
        exp1 = pow(m, -2)*((-1)*k*k*y0 + (-4)*e*k*k*pow(y0, 3) + (-3)*pow(e, 2)*pow(k, 2)*pow(y0, 5) + (-6)*e*k*m*y0*pow(y1, 2) + pow(c, 2)*y2 + (-2)*k*m*y2 + (-6)*e*k*m*pow(y0, 2)*y2);
...
__device__ double * rk4(double xx, double yy, double *resp, int pi, int pf){
...
__device__ double * numjac(double *resp, double xx, double yy, double yb1, double yb2, int Nt0, int pi, int pf){
...
__device__ double bvpsol(double *resp, double *J, double yb1, double yb2, int Nt0, int pi, int pf){
...
//Kernel
__global__ void shoot(double *c, double *X1, double *X2, double *resp, double *J, int *Nt){
...
int main(void){
    ...
    double *d_c, *d_X1, *d_X2, *d_resp, *d_J;
    int *d_Nt;
    ...
    cudaMemcpy(d_Nt, &Nt, size_int,cudaMemcpyHostToDevice);

    // Launch kernel on GPU
    cudaEventRecord(start);
    shoot<<<2,512>>>(d_c, d_X1, d_X2, d_resp, d_J, d_Nt);  
    ...
    cudaFree(d_c); cudaFree(d_X1); cudaFree(d_X2); cudaFree(d_resp); cudaFree(d_J);
